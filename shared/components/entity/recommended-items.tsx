...
export default function RecommendedItems({ currentId, type }: RecommendedItemsProps) {
  ...
  const animateTransition = (newItems: any[], direction: 'left' | 'right') => {
    ...
    // Slower animation timing - 800ms total duration
    setTimeout(() => {
      setCurrentItems(newItems);
      requestAnimationFrame(() => {
        setTimeout(() => {
          setIsAnimating(false);
        }, 100); // Small delay before removing animation class
      });
    ...
    <section className="border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 sm:p-6 bg-white dark:bg-zinc-950/50 mb-8">
      ...
      <div className="relative overflow-hidden">
        <div className={cn(
          ...
          "transition-all duration-700 ease-in-out transform", // Increased duration
          isAnimating && slideDirection === 'left' && "-translate-x-[120%] opacity-0",
          isAnimating && slideDirection === 'right' && "translate-x-[120%] opacity-0",
